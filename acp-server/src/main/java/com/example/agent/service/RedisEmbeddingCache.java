...
public class RedisEmbeddingCache implements EmbeddingCache {
    ...
    private String key(String tenantId, String model, String schemaVersion, String textHash) {
        return "emb:cache:" + tenantId + ":" + model + ":v" + schemaVersion + ":" + textHash;
    }

    @Override
    public Mono<float[]> get(String tenantId, String model, String schemaVersion, String textHash) {
        return cmd.get(key(tenantId, model, schemaVersion, textHash))
                .map(VectorCodec::fromBytes)
                .switchIfEmpty(Mono.empty());
    }

    @Override
    public Mono<Void> put(String tenantId, String model, String schemaVersion, String textHash, float[] vector, Duration ttl) {
        String k = key(tenantId, model, schemaVersion, textHash);
