...
public class DefaultPromptAssembler implements PromptAssembler {
    ...
    private String formatFacts(List<Fact> facts) {
        if (facts == null || facts.isEmpty()) return "- (none)\n";
        StringBuilder sb = new StringBuilder();
        for (Fact f : facts) {
    ...
    private String formatToolReceipts(List<ToolReceipt> receipts) {
        if (receipts == null || receipts.isEmpty()) return "- (none)\n";
        StringBuilder sb = new StringBuilder();
    ...
    private String outputSchema() {
        ...
        {
          "decision": {
            "action": "SEND|SUPPRESS|SCHEDULE|ESCALATE",
            "channel": "SMS|EMAIL|WHATSAPP|PUSH|NONE",
            "sendAt": "ISO-8601 or null",
    ...
    private PromptPackage enforceBudget(int totalBudget, PromptPackage pkg, ContextBundle bundle, SectionCaps caps) {
        ...
        if (est <= totalBudget) return pkg;

        // If over budget: prune memory (episodic first, then semantic), keep facts + procedural
        ...
        pages.sort((a, b) -> {
            ...
            if (pa != pb) return Integer.compare(pb, pa); // higher = drop sooner
        ...
        for (MemoryPage p : pages) {
            if (p.pageType() == PageType.PROCEDURAL) kept.add(p);
        }

        // then add semantic until cap, then episodic until cap
        for (PageType t : List.of(PageType.SEMANTIC, PageType.EPISODIC)) {
            for (MemoryPage p : pages) {
                if (p.pageType() != t) continue;
                kept.add(p);
                ContextBundle tmp = new ContextBundle(bundle.facts(), kept, bundle.toolReceipts(), bundle.provenance(), bundle.tokenEstimate());
        ...
        ContextBundle shrunk = new ContextBundle(bundle.facts(), kept, bundle.toolReceipts(), bundle.provenance(), bundle.tokenEstimate());
    ...
    private String buildUserDummyForBudget(ContextBundle bundle, SectionCaps caps) {
        // Minimal rebuild for budget enforcement. (In real implementation, call buildUser with request too.)
        StringBuilder sb = new StringBuilder();
        sb.append("## Deterministic Facts\n")
          .append(limitToCap(formatFacts(bundle.facts()), caps.factsTokens)).append("\n\n");
        sb.append("## Procedural Rules\n")
          .append(limitToCap(formatPages(bundle.pages(), PageType.PROCEDURAL), caps.proceduralTokens)).append("\n\n");
        sb.append("## Semantic Memory\n")
