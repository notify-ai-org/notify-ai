...
public class DefaultRetrievalPlanner implements RetrievalPlanner {
    ...
    public DefaultRetrievalPlanner(FactStore factStore, PageStore pageStore, VectorIndex vectorIndex, TokenEstimator tokenEstimator) {
    ...
    public ContextBundle plan(DecisionRequest req) {
        ...
        // Reserve some room for PromptAssembler instruction/schema and model output buffer
        ...
        for (RetrievalQuery q : queries) {
            candidates.addAll(vectorIndex.search(
                    req.tenantId(),
                    q.queryText(),
                    q.scope(),
    ...
    private int estimateFactsTokens(List<Fact> facts) {
        int sum = 0;
        for (Fact f : facts) sum += tokenEstimator.estimateTokens(f.sentence());
        return sum;
    }

    private int estimatePagesTokens(List<MemoryPage> pages) {
        int sum = 0;
        for (MemoryPage p : pages) sum += tokenEstimator.estimateTokens(p.summary());
        return sum;
    }
    ...
    private List<RetrievalQuery> buildQueries(DecisionRequest req) {
        ...
        return switch (req.decisionType()) {
            case CHANNEL_FALLBACK -> List.of(
                    new RetrievalQuery("user channel preference and engagement patterns", Set.of("preference","engagement","channel"), scope, PageType.SEMANTIC, 25),
                    new RetrievalQuery("recent delivery failures and suppression reasons", Set.of("failure","suppression","retry"), scope, PageType.EPISODIC, 25),
                    new RetrievalQuery("tenant provider reliability incidents", Set.of("incident","provider","outage"), List.of(new EntityRef("TENANT", req.tenantId())), PageType.SEMANTIC, 25),
                    new RetrievalQuery("successful fallback patterns after similar failures", Set.of("fallback","success"), scope, PageType.SEMANTIC, 25)
            );
            case SCHEDULE -> List.of(
                    new RetrievalQuery("preferred send windows and do-not-disturb behavior", Set.of("schedule","dnd","window"), scope, PageType.SEMANTIC, 25),
                    new RetrievalQuery("recent suppressions and reschedules", Set.of("suppression","reschedule"), scope, PageType.EPISODIC, 25)
            );
            case SUPPRESS -> List.of(
                    new RetrievalQuery("suppression reasons and complaints history", Set.of("complaint","optout","suppression"), scope, PageType.EPISODIC, 25),
                    new RetrievalQuery("policy rules for suppression and compliance", Set.of("policy","compliance"), List.of(new EntityRef("TENANT", req.tenantId())), PageType.PROCEDURAL, 15)
            );
            case TEMPLATE_PICK -> List.of(
                    new RetrievalQuery("template performance and engagement by channel", Set.of("template","engagement"), scope, PageType.SEMANTIC, 25),
                    new RetrievalQuery("recent template failures and rendering issues", Set.of("template","failure"), scope, PageType.EPISODIC, 25)
            );
            case ESCALATE -> List.of(
                    new RetrievalQuery("escalation rules and severity thresholds", Set.of("escalation","severity"), List.of(new EntityRef("TENANT", req.tenantId())), PageType.PROCEDURAL, 15),
                    new RetrievalQuery("similar escalations and outcomes", Set.of("escalation","outcome"), scope, PageType.EPISODIC, 25)
            );
        };
    }
    ...
    private double decisionMatchScore(Set<String> tags, DecisionType dt) {
        ...
        Set<String> wanted = switch (dt) {
            case CHANNEL_FALLBACK -> Set.of("fallback","failure","provider","channel","retry","engagement","preference");
    ...
    private double scopeMatchScore(List<EntityRef> pageScope, List<EntityRef> reqScope) {
        if (pageScope == null || pageScope.isEmpty()) return 0.2;
