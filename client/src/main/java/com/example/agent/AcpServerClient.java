...
/**
 * HTTP client for acp-server: registration, token refresh, vocabulary, rules, event captures.
 * Handles auth via Bearer token. Caller is responsible for token refresh on 401.
 */
public class AcpServerClient {
     ...
     */
    public ClientRegistrationDto.Response register(ClientRegistrationDto.Request req, String bearerToken) throws Exception {
        String json = mapper.writeValueAsString(req);
        HttpRequest.Builder b = HttpRequest.newBuilder()
            .uri(URI.create(baseUrl + "/api/client/register"))
        ...
        if (r.statusCode() >= 400) throw new RuntimeException("Register failed: " + r.statusCode() + " " + r.body());
    ...
    public TokenRefreshDto.Response refreshToken(TokenRefreshDto.Request req) throws Exception {
        ...
        HttpResponse<String> r = httpClient.send(
            HttpRequest.newBuilder()
                .uri(URI.create(baseUrl + "/api/auth/token/refresh"))
                .header("Content-Type", "application/json")
        ...
        if (r.statusCode() >= 400) throw new RuntimeException("Token refresh failed: " + r.statusCode() + " " + r.body());
